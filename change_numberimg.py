...
# 指定路径
path = 'E:/car/material_tf/image/Letter'
# 指定新文件的路径
new_path = 'E:/car/material_tf/image/Letter_split'
# 指定背景图路径
    ...
    if os.path.isdir(folder_path):  # 确保是文件夹
        ...
        for filename in tqdm(os.listdir(folder_path), desc=f'Processing images in {foldername}'):
            if filename.endswith('.jpg'):
                ...
                # 裁剪图片
                img = img[top_left_y:top_left_y+height, top_left_x:top_left_x+width]

                # 定义新的文件名和路径
                base, extension = os.path.splitext(filename)  # 分离文件名和扩展名
                new_filename = base + '_B' + extension  # 新文件名
