...
training_set = train_datagen_augment.flow_from_directory('image_split/Material_split/train',
                                                        target_size = (INPUT_SIZE, INPUT_SIZE),
                                                        batch_size = 32)

training_set_fine = train_datagen_noaugment.flow_from_directory('image_split/Material_split/train',
                                                                target_size = (INPUT_SIZE, INPUT_SIZE),
                                                                batch_size = 32)

test_set = test_datagen.flow_from_directory('image_split/Material_split/test',
                                            target_size = (INPUT_SIZE, INPUT_SIZE),
                                            batch_size = 32)
...
fine_tune_epochs = 35 # 继续训练
total_epochs =  initial_epochs + fine_tune_epochs

history_fine = model.fit(training_set_fine,
                         epochs=total_epochs,
...
quant_set = test_datagen.flow_from_directory('image_split/Material_split/test',
                        target_size = (INPUT_SIZE, INPUT_SIZE),
                        batch_size = 1)
...
model_dir = './model'
tflite_model_path = os.path.join(model_dir, 'trained_material_demo2.tflite')
